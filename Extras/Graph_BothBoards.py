...
@app.callback(Output('flowrate-dropdown', 'options'),
              Input('stage-dropdown', 'value'), Input('substance-dropdown', 'value'))
...
def update_voltage_graph(flow, stage, substance):
    ...
    # Split voltage columns by board
    volt_cols_b1 = [c for c in df.columns if c.startswith("B1") and c.endswith("(V)")]
    volt_cols_b2 = [c for c in df.columns if c.startswith("B2") and c.endswith("(V)")]

    for col in volt_cols_b1 + volt_cols_b2:
        sub_df[col] = pd.to_numeric(sub_df[col].astype(str).str.replace(" V", "", regex=False), errors='coerce')

    bme_map = {
        "BME688 - temperature (°C)": (3, "Temperature (°C)"),
        "BME688 - humidity (%)": (4, "Humidity (%)"),
        "BME688 - pressure (KPa)": (5, "Pressure (kPa)")
    }

    fig = make_subplots(
        rows=5, cols=1, shared_xaxes=True, vertical_spacing=0.05,
        row_heights=[0.35, 0.35, 0.1, 0.1, 0.1],
        subplot_titles=[
            f"B1 Voltage vs Time @ {flow} L/min",
            f"B2 Voltage vs Time @ {flow} L/min",
            "Temperature", "Humidity", "Pressure"
        ]
    )

    def clean_label(label):
        label = re.sub(r"B[12] - .*? - ", "", label)
        return label.replace(" (V)", "")

    for col in volt_cols_b1:
        fig.add_trace(go.Scatter(x=sub_df["Timestamp"], y=sub_df[col],
                                 mode="lines+markers", name=clean_label(col)),
                      row=1, col=1)
    for col in volt_cols_b2:
        fig.add_trace(go.Scatter(x=sub_df["Timestamp"], y=sub_df[col],
                                 mode="lines+markers", name=clean_label(col)),
                      row=2, col=1)
    ...
    for name, (r, label) in bme_map.items():
        if name in df.columns:
            fig.add_trace(
                go.Scatter(x=df["Timestamp"], y=df[name], mode="lines", name=label,
                           line=dict(shape='spline')),
                row=r, col=1
            )

    if sub_df["Timestamp"].notnull().any():
        x_min = sub_df["Timestamp"].min()
        x_max = sub_df["Timestamp"].max()
        for i in range(1,6):
    ...
    fig.update_yaxes(title_text="kPa", row=5, col=1)

    return fig

# === SIMPLE PPM/PPB GRAPH PAGE ===
@app.callback(Output('ppm-graph-only', 'figure'),
    Input('flowrate-dropdown', 'value'),
    State('stage-dropdown', 'value'),
    State('substance-dropdown', 'value'))
def update_ppm_graph(flow, stage, substance):
    ...
    fig.update_layout(
        title=f"{stage} - {substance} | PPM/PPB @ {flow} L/min",
        xaxis_title="Time", yaxis_title="Concentration (ppm/ppb)",
        height=700
    )
    return fig
